...
import { ExternalLink, Heart, LocateFixed, Navigation, Share2 } from 'lucide-react';
            ...
            <InfoWindowF position={{ lat: selectedService.latitude, lng: selectedService.longitude }} onCloseClick={() => onSelectService(null)} options={{ pixelOffset: new window.google.maps.Size(0, -24) }}>
              <div className="max-w-[240px] p-1 text-slate-900">
                ...
                <div className="mt-3 grid grid-cols-2 gap-1.5">
                  <button type="button" onClick={() => onToggleFavorite(selectedService)} className="inline-flex items-center justify-center gap-1 rounded-lg bg-rose-600 px-2 py-2 text-xs font-semibold text-white"><Heart className={`h-3.5 w-3.5 ${favoriteIds?.has(selectedService.placeId) ? 'fill-current' : ''}`} /> Save</button>
