...
export const adminApi = {
  ...
  analyticsUsers: (params) => adminHttp.get('/analytics/users', { params }),
  analyticsSos: (params) => adminHttp.get('/analytics/sos', { params }),
  analyticsLocation: (params) => adminHttp.get('/analytics/location', { params }),
  analyticsEmail: (params) => adminHttp.get('/analytics/email', { params }),
  analyticsNotifications: (params) => adminHttp.get('/analytics/notifications', { params }),
  analyticsSystem: (params) => adminHttp.get('/analytics/system', { params }),
  reports: (params) => adminHttp.get('/reports', { params }),
  exportReports: (format = 'csv', filters = {}) => adminHttp.post('/reports/export', { format, filters }, { responseType: 'blob' }),
  auditLogs: (params) => adminHttp.get('/audit-logs', { params }),
  globalSearch: (params) => adminHttp.get('/search', { params }),
  sos: (params) => adminHttp.get('/sos', { params }),
  sosDetails: (id) => adminHttp.get(`/sos/${id}`),
  sosTimeline: (id) => adminHttp.get(`/sos/${id}/timeline`),
  sosTracking: (id) => adminHttp.get(`/sos/${id}/tracking`),
  reviewSos: (id) => adminHttp.patch(`/sos/${id}/review`),
  updateSosNotes: (id, notes) => adminHttp.patch(`/sos/${id}/notes`, { notes }),
  exportSos: (format = 'csv', filters = {}) => adminHttp.post('/sos/export', { format, filters }, { responseType: 'blob' }),
  emailLogs: (params) => adminHttp.get('/email/logs', { params }),
  emailLogDetails: (id) => adminHttp.get(`/email/logs/${id}`),
  emailQueue: (params) => adminHttp.get('/email/queue', { params }),
  emailStatistics: (params) => adminHttp.get('/email/statistics', { params }),
  retryEmail: (id) => adminHttp.post(`/email/retry/${id}`),
  retryFailedEmails: (limit = 25) => adminHttp.post('/email/retry-failed', { limit }),
  exportEmailLogs: (format = 'csv', filters = {}) => adminHttp.post('/email/export', { format, filters }, { responseType: 'blob' }),
  settings: () => adminHttp.get('/settings'),
  updateSettings: (payload) => adminHttp.put('/settings', payload),
  updateGeneralSettings: (payload) => adminHttp.patch('/settings/general', payload),
  updateSmtpSettings: (payload) => adminHttp.patch('/settings/smtp', payload),
  updateThemeSettings: (payload) => adminHttp.patch('/settings/theme', payload),
  updateApplicationSettings: (payload) => adminHttp.patch('/settings/application', payload),
  updateSecuritySettings: (payload) => adminHttp.patch('/settings/security', payload),
  updateMaintenanceSettings: (payload) => adminHttp.patch('/settings/maintenance', payload),
  emailTemplates: () => adminHttp.get('/settings/email-templates'),
  updateEmailTemplate: (id, payload) => adminHttp.patch(`/settings/email-template/${id}`, payload),
  testEmailTemplate: (payload) => adminHttp.post('/settings/email-template/test', payload),
  settingsEmergencyNumbers: (params) => adminHttp.get('/settings/emergency-numbers', { params }),
  createSettingsEmergencyNumber: (payload) => adminHttp.post('/settings/emergency-numbers', payload),
  updateSettingsEmergencyNumber: (id, payload) => adminHttp.patch(`/settings/emergency-numbers/${id}`, payload),
  deleteSettingsEmergencyNumber: (id) => adminHttp.delete(`/settings/emergency-numbers/${id}`),
  testSmtpSettings: () => adminHttp.post('/settings/smtp/test'),
